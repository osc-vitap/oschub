...
def populateEventUserData(num=1000):
    ...
    emailList = []
    for i in range(num):
        emailList.append(nameList[i].split(' ')[0].lower() + '.' + regList[i].lower() + '@vitap.ac.in')
    ...
    for event in [temp for temp in eventList if temp not in completedEvents]:
        for eventData in Event.objects.filter(eventName=event):
            for i in range(num):
                student = [eventData, nameList[i], regList[i], emailList[i], True, random.choice([True, False])]
...
if __name__ == '__main__':
    ...
    for data in modelData:
        row = EventUserData(eventName=data[0], studentName=data[1], studentReg=data[2], studentEmail=data[3], studentRegistered=data[4], studentCheckedIn=data[5])
        row.save()
